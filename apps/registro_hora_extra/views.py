...
class Hora_extraEditBase(UpdateView):
    model = RegistroHoraExtra
    ...
    def get_form_kwargs(self):
        kwargs = super(Hora_extraEditBase, self).get_form_kwargs()
        kwargs.update({'user': self.request.user})
        return kwargs
...
class UtilizouHoraExtra(View):
    def post(self, *args, **kwargs):
        ...
        if registro_hora_extra.utilizada ==True:
...
class ExportarParaCsv(View):
    def get(self, request):
        ...
        registro_hr = RegistroHoraExtra.objects.filter(utilizada=False)

        writer = csv.writer(response)

        writer.writerow(['id', 'motivo', 'Nome', 'restante_func', 'horas'])
        for registro in registro_hr:
